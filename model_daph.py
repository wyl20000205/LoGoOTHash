...
class DPDH_Encoder(nn.Module):
    ...
    def _encode_image(self, image):
        ...
        # print(image.shape)
        image_embed_1 = self.clip.encode_image(image, image_prompt)  # bs 768
        image_pre = self.image_pre(image_embed_1)
        return image_embed_1, image_pre  # bs 32

    def _encode_text(self, text, text_prompt):
        text_embed_1, prompt_embed_1 = self.clip.encode_text(
            text, text_prompt
        )  # bs 768
        text_pre = self.text_pre(text_embed_1)
        return text_embed_1, text_pre, prompt_embed_1
    ...
    def forward(self, image, text, key_padding_mask, label):
        ...
        class_names_prompt = [
            [self.class_name_list[j] for j in indices[i]] for i in range(B)
        ]
        ...
        text_features, text_pre, prompt_embed_1 = self._encode_text(text, text_prompt)
        ...
        image_features, text_features = self.grn(
        ...
        # plt.scatter(X_2d[:, 0], X_2d[:, 1], c='blue', alpha=0.6, s=30)
